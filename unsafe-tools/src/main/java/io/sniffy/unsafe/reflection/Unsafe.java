    ...
    public static int getPid() {
        try {
            ...
            if (Unsafe.getJavaVersion() >= 9) {
                ...
                Object vmManagement = $(ManagementFactory.getRuntimeMXBean().getClass(), Object.class).getNonStaticField("jvm").get(ManagementFactory.getRuntimeMXBean());
                int pid = $(vmManagement.getClass(), Object.class).getNonStaticMethod(Integer.TYPE, "getProcessId").invoke(vmManagement);
                return pid;
            }
                ...
                if (null == instrumentationFuture || instrumentationFuture.isCancelled()) {
                    instrumentationFuture = attachAgentExecutor.submit(new Callable<Instrumentation>() {
                        ...
                        public Instrumentation call() throws Exception {
                            ...
                            manifest.getMainAttributes().put(new Attributes.Name("Agent-Class"), "io.sniffy.unsafe.agent.UnsafeAgent");
                            ...
                            JarEntry jarEntry = new JarEntry("io/sniffy/unsafe/agent/UnsafeAgent.class");
                            jarOutputStream.putNextEntry(jarEntry);

                            InputStream resourceAsStream = Main.class.getClassLoader().getResourceAsStream("io/sniffy/unsafe/agent/UnsafeAgent.class");
                            ...
                            while(!Thread.currentThread().isInterrupted()){
                                //do something
                                UnresolvedClassRef<Object> classRef = $("io.sniffy.unsafe.agent.UnsafeAgent");
